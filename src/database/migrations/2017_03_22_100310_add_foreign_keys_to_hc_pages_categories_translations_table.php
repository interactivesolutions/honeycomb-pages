...
class AddForeignKeysToHcPagesCategoriesTranslationsTable extends Migration {
	...
	public function up()
	{
		Schema::table('hc_pages_categories_translations', function(Blueprint $table)
		{
			...
			$table->foreign('record_id', 'fk_hc_pages_categories_translations_hc_pages_categories1')->references('id')->on('hc_pages_categories')->onUpdate('NO ACTION')->onDelete('NO ACTION');
			$table->foreign('cover_photo_id', 'fk_hc_pages_categories_translations_hc_resources1')->references('id')->on('hc_resources')->onUpdate('NO ACTION')->onDelete('NO ACTION');
		});
	}
